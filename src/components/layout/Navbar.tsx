  ...
  return (
    <nav className="sticky top-0 z-50 flex items-center justify-between px-4 py-3 bg-white border-b border-gray-100 shadow-sm">
      ...
      <div className="flex items-center gap-4">
        <button onClick={onOpenSidebar} className="p-2 hover:bg-gray-100 rounded-full transition-colors cursor-pointer">
      ...
      <div className="hidden lg:flex items-center gap-6">
        ...
        <div className="flex items-center bg-gray-100 p-1 rounded-full relative">
          <button 
            ...
            className={`px-5 py-2 rounded-full font-semibold text-sm transition-all duration-300 ease-in-out cursor-pointer ${
              orderType === 'delivery' 
      ...
      <div className="flex items-center gap-2 md:gap-4">
        ...
        {isLoggedIn ? (
          <>
            <Link to="/profile" className="hidden sm:flex items-center gap-2 px-3 py-2 font-semibold text-sm hover:bg-gray-100 rounded-full transition-colors text-black cursor-pointer">
            ...
            <button onClick={onOpenCart} className="p-2 hover:bg-gray-100 rounded-full relative transition-colors ml-1 cursor-pointer">
              ...
              {cartItemCount > 0 && (
                <span className="absolute top-0 right-0 inline-flex items-center justify-center px-1.5 py-0.5 text-[10px] font-bold leading-none text-white transform translate-x-1/4 -translate-y-1/4 bg-[#d81b60] rounded-full border-2 border-white">
                  {cartItemCount}
                </span>
              )}
            </button>
          ...
          <>
            <button onClick={onOpenLogin} className="hidden sm:block px-4 py-2 font-semibold text-sm hover:bg-gray-100 rounded-full transition-colors text-black cursor-pointer">Log in</button>
            <button onClick={onOpenSignUp} className="px-5 py-2 font-semibold text-sm bg-black text-white rounded-full hover:bg-gray-800 transition-colors cursor-pointer">Sign up</button>
            <button onClick={onOpenCart} className="p-2 hover:bg-gray-100 rounded-full relative transition-colors ml-1 cursor-pointer">
              <svg className="w-6 h-6 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
              {cartItemCount > 0 && (
                <span className="absolute top-0 right-0 inline-flex items-center justify-center px-1.5 py-0.5 text-[10px] font-bold leading-none text-white transform translate-x-1/4 -translate-y-1/4 bg-[#d81b60] rounded-full border-2 border-white">
